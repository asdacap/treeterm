...
export class SSHTunnel {
  ...
  private async bootstrapRemoteDaemon(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      const bootstrapScript = [
        ...
        'fi',
        '',
        '# Detect stale socket (file exists but daemon process is dead)',
        'if [ -S "$DAEMON_SOCKET" ] && ! pgrep -x "treeterm-daemon" > /dev/null 2>&1; then',
        '  echo "Stale daemon socket detected (process not running), cleaning up..."',
        '  rm -f "$DAEMON_SOCKET"',
        'fi',
        '',
      ...
      proc.on('close', (code) => {
        if (code !== 0) {
          reject(new Error(`SSH bootstrap failed (exit ${String(code)}). See log output for details.`))
          return
        }
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        if (code !== 0) {
          ...
          reject(new Error(`SSH command failed (exit ${String(code)}): ${firstLine}`))
        } else {
          resolve(stdout)
        }
      })
  ...
  private startTunnel(remoteSocketPath: string): void {
    const sshArgs = this.buildBaseSSHArgs()
