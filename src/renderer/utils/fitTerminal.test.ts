...
describe('fitTerminal', () => {
  ...
  it('returns early when cellWidth is 0', () => {
    const onResize = vi.fn<(cols: number, rows: number) => void>()
    fitTerminal(makeTerminal({ cellWidth: 0 }), onResize, mockComputeStyle)
    expect(onResize).not.toHaveBeenCalled()
  })

  it('returns early when cellHeight is 0', () => {
    const onResize = vi.fn<(cols: number, rows: number) => void>()
    fitTerminal(makeTerminal({ cellHeight: 0 }), onResize, mockComputeStyle)
    expect(onResize).not.toHaveBeenCalled()
  })
  ...
  it('returns early when parentElement is null', () => {
    const onResize = vi.fn<(cols: number, rows: number) => void>()
    fitTerminal(makeTerminal({ hasParent: false }), onResize, mockComputeStyle)
    expect(onResize).not.toHaveBeenCalled()
  })

  it('returns early when container has zero width', () => {
    const onResize = vi.fn<(cols: number, rows: number) => void>()
    fitTerminal(makeTerminal({ rectWidth: 0 }), onResize, mockComputeStyle)
    expect(onResize).not.toHaveBeenCalled()
  })

  it('returns early when container has zero height', () => {
    const onResize = vi.fn<(cols: number, rows: number) => void>()
    fitTerminal(makeTerminal({ rectHeight: 0 }), onResize, mockComputeStyle)
    expect(onResize).not.toHaveBeenCalled()
  })
  ...
  it('subtracts padding from available space', () => {
    const paddedStyle = vi.fn<(elt: Element) => CSSStyleDeclaration>().mockReturnValueOnce({
      paddingLeft: '10',
      paddingRight: '10',
      paddingTop: '8',
      paddingBottom: '8',
    ...
    fitTerminal(makeTerminal(), onResize, paddedStyle)
