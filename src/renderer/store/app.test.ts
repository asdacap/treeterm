...
const mockDeps = {
  ...
  github: { getPrInfo: vi.fn<(...args: any[]) => any>(), listOpenPrs: vi.fn<(...args: any[]) => any>(), postReviewComments: vi.fn<(...args: any[]) => any>() },
  filesystem: {},
  reviews: {},
  stt: {},
...
describe('useAppStore', () => {
  ...
  describe('initialize', () => {
    ...
    it('localConnect creates store for session with no workspaces', async () => {
      ...
      vi.mocked(mockDeps.appApi.localConnect).mockResolvedValue({
        info: localConn,
        session: { id: 'empty-session', workspaceRefs: [], workspaceDataDir: "", createdAt: 0, lastActivity: 0, version: 1 }
      })
      const cleanup = await useAppStore.getState().initialize(mockDeps)
  ...
  describe('handleExternalSessionUpdate via onSync', () => {
    ...
    it('delegates external update handling to session store', () => {
      const session: any = {
        id: 'sync-session-2',
        workspaceRefs: [] // No workspaces — removal handled inside session store
      }
    ...
    it('ignores onSync for unknown connection', () => {
      const session: any = {
        id: 'unknown-session',
        workspaceRefs: [{ id: 'ws-1', path: '/test', name: 'test' }]
      }
    ...
    it('routes onSync to correct session by connection ID', () => {
      const session: any = { id: 'sync-session-3', workspaceRefs: [] }
  ...
  describe('application registry', () => {
    ...
    it('registerAiHarnessVariants registers AI apps', async () => {
      ...
      useAppStore.getState().registerAiHarnessVariants([
        { id: 'claude', name: 'Claude', icon: 'C', command: 'claude', isDefault: false, enableSandbox: false, allowNetwork: true, backgroundColor: '#000', disableScrollbar: false, keepOnExit: false }
      ])
      cleanup()
    })
  ...
  describe('session naming', () => {
    ...
    describe('local session via localConnect', () => {
      it('does not persist a default name — LOCAL is derived from the connection', async () => {
        ...
        vi.mocked(mockDeps.appApi.localConnect).mockResolvedValue({
          info: localConn,
          session: { id: 'local-session-1', workspaceRefs: [], workspaceDataDir: "", createdAt: 0, lastActivity: 0, version: 1 }
        })
        const cleanup = await useAppStore.getState().initialize(mockDeps)
      ...
      it('does not overwrite existing custom name on same session', async () => {
        ...
        vi.mocked(mockDeps.appApi.localConnect).mockResolvedValue({
          info: localConn,
          session: { id: 'local-session-2', workspaceRefs: [], workspaceDataDir: "", createdAt: 0, lastActivity: 0, version: 1 }
        })
        const cleanup = await useAppStore.getState().initialize(mockDeps)
        const storeKey = Array.from(useAppStore.getState().sessionStores.keys())[0]!
    ...
    describe('SSH session via addRemoteSession', () => {
      ...
      it('does not overwrite existing custom name set via startRemoteConnect', async () => {
        ...
        const session = { id: 'ssh-session-3', workspaceRefs: [], workspaceDataDir: "", createdAt: 0, lastActivity: 0, version: 1 }
        await useAppStore.getState().addRemoteSession(session, connection)
