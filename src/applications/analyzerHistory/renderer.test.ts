...
vi.mock('../../renderer/components/AnalyzerHistory', () => ({
  default: vi.fn(() => null)
}))
...
const mockWorkspaceStore = createStore<WorkspaceStoreState>()(() => ({
  ...
  initAnalyzer: vi.fn(), createTty: vi.fn().mockResolvedValue('pty-1'), ensureTtyForTab: vi.fn().mockResolvedValue('pty-1'), getTtyWriter: vi.fn().mockResolvedValue({ write: vi.fn<(data: string) => void>(), kill: vi.fn<() => void>() }),
...
describe('AnalyzerHistory Renderer', () => {
  ...
  it('has correct application properties', () => {
    ...
    expect(analyzerHistoryApplication.isDefault).toBe(false)
  })

  it('creates empty initial state', () => {
    expect(analyzerHistoryApplication.createInitialState()).toEqual({})
  })
