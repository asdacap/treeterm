...
- **Renderer** (React): UI components, Zustand state, and business logic. High-level business logic.
- **Main** (Electron): IPC handlers, light proxy to daemon. Because there can be multiple window per electron main process. Its easier to have the bulk of the logic handled within the renderer. This means that the main generally will provide synthetic autogenerated id for each grpc cliend and the renderer send back the id to identify which grpc client it is interacting with.
...
- Make test. Focus on branch coverage. Untestable branch is likely removable branch or out of scope or impossible case.

### Do not store information that can be re-calculated when needed.
- For example, do not store parent branch name. This is because parent branch can be calculated using the parent id. 
- If parent branch is stored, then if parent id changed, then parent branch also need to change which adds failure point and more cored.
